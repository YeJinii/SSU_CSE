...
long long getRssShmem(int pid){
	...
	memset(line_tmp,  '\0', MAXLEN);
	while(NULL!=fgets(line_tmp, MAXLEN, status_fp)){
		if(strncmp(line_tmp, "RssShmem",8)==0){
			check=1;
			break;
		}
		else {
...
void makehead(){
	...
	long long int memFree = getMemFree();		//%CPU 계산

	long long int memUsed = memTotal-memFree-getBuffer()-getCache();
	long long int membc = getBuffer()+getCache()+getSR();

	long long int swapTotal = getSwapTotal();
	long long int swapFree = getSwapFree();
	long long int swapUsed = swapTotal-swapFree;		//%CPU 계산

	long long int MemAvail = getMemAvail();
	...
	sprintf(head[3],"KiB Mem : %8lld total, %8lld free, %8lld used, %8lld buff/cache", memTotal, memFree, memUsed, membc);
	sprintf(head[4],"KiB Swap: %8lld total, %8lld free, %8lld used, %8lld avail Mem", swapTotal, swapFree, swapUsed, MemAvail);
